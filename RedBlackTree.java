...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  protected static class Node<T> {
    ...
    /**
     * @return true when this node has a parent andd is the left child of that parent, otherwise
     *         return false
     */
  ...
  private void enforceRBTreePropertiesAfterInsert(Node<T> newNode) {
    // New Node is the only node in the tree, so no RBT Violations, return
    if (newNode.parent == null) {
      return;
    }

    // New Node's parent is Black, nothing has to be done so return
    if (newNode.parent.isBlack == true) {
      return;

      // New Node's parent is Red, break into cases to resolve
    } else {
      // Makes sure the grandparent is not null
      if (newNode.parent.parent != null) {
        Node<T> grandparent = newNode.parent.parent;
        Node<T> sibling;

        // Gets the sibling of the newNode (aka Uncle)
        ...
        // If the sibling is null or is colored Black
        if (sibling == null || sibling.isBlack) {
          if (newNode.parent.isLeftChild()) {
            if (newNode.parent.rightChild == newNode) {
              // Checks for the triangle that requires 2 rotations (Parent is Left Child and NewNode
              // is Right Child of Parent ie Triangle)
              ...
              return;
              // Case that only requires one rotation because nodes are in a straight line
            ...
            if (newNode.parent.leftChild == newNode) {
              // Triangle case that requires 2 rotations (Parent is Right Child and NewNode is Left
              // Child of parent ie Triangle)
              rotate(newNode, newNode.parent);
              System.out.println(newNode.parent.equals(grandparent));
              rotate(newNode, grandparent);
              grandparent.isBlack = false;
              if (newNode != root) {
                newNode.isBlack = true;
        ...
        if (sibling.isBlack == false) {
          // Recolor should fix the RBT Property Violations
          newNode.parent.isBlack = true;
          sibling.isBlack = true;
          grandparent.isBlack = false;

          // Recursive call that enforces the cascading fix if grandparent's parent is now red
          // (double red RBT Violation)
          if (grandparent != root && !grandparent.parent.isBlack) {
  ...
  private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
    // Checks if the child is a left or right child of parent
    if (!child.isLeftChild() && !child.parent.rightChild.equals(child)) {
      throw new IllegalArgumentException("Not a left or right child of parent node.");
    }

    // Parent node is the root node
    if (root == parent) {
      // Left child so Right Rotation
    ...
    // Left Child so Right Rotation
    if (child.isLeftChild()) {
      // Adjusts parent's parent child accordingly
      ...
      // If child has its own right child
      if (child.rightChild != null) {
        parent.leftChild = child.rightChild;
      } else {
        parent.leftChild = null;
      ...
      if (parent.isLeftChild()) {
        parent.parent.leftChild = child;
        child.parent = parent.parent.leftChild;
      } else if (parent.parent.rightChild.equals(parent)) {
        parent.parent.rightChild = child;
        child.parent = parent.parent;
      }
